...
void VSE_SetShaderUniform(GLuint shaderProgram, char *propertyName, VSE_UniformType uniformType,
                      void *value)
{
...
void VSE_AddPostProcessingEffect(VSE_Engine *engine, char *effectName, VSE_Material *material)
{
	if (material == NULL)
	{
		SDL_Log("VSE_AddPostProcessingEffect: material is NULL for effect '%s'", effectName);
		return;
	}

	VSE_DictionaryAdd(engine->renderer->postProcessingEffects, effectName, material);
}
...
static void RenderTexture(VSE_Renderer *renderer, VSE_Texture *texture, float x, float y, float width,
                   float height)
{
	float verticesData[] = {
		x, y, 0.0f, 0.0f,
...
static void RenderSprite(VSE_Engine *engine, VSE_Vector2Float screenPosition, VSE_Entity *entity,
                  VSE_SpriteRenderer *sprite, float projectionMatrix[16])
{
	sprite->size.x = sprite->originalSize.x * engine->pixelsPerUnit * entity->transform.scale.x;
	sprite->size.y = sprite->originalSize.y * engine->pixelsPerUnit * entity->transform.scale.y;

	float x = screenPosition.x - sprite->size.x / 2.0f;
	float y = screenPosition.y - sprite->size.y / 2.0f;

	glUseProgram(sprite->material->shaderProgram);
	VSE_UniformTypeValuePair *typeValuePair = VSE_DictionaryGet(sprite->material->materialUniforms,
	                                                    "projection");
	typeValuePair->uniformValue = projectionMatrix;
	VSE_UpdateMaterialUniforms(sprite->material);
	RenderTexture(engine->renderer, sprite->texture, x, y, sprite->size.x, sprite->size.y);
}


static void RenderUIImage(VSE_Engine *engine, VSE_Entity *entity, VSE_SpriteRenderer *image,
                          VSE_RectTransform *rect, float projectionMatrix[16])
{
	...
	VSE_UniformTypeValuePair *typeValuePair = VSE_DictionaryGet(image->material->materialUniforms,
	                                                            "projection");
	typeValuePair->uniformValue = projectionMatrix;
	VSE_UpdateMaterialUniforms(image->material);
	RenderTexture(engine->renderer, image->texture, x, y, image->size.x, image->size.y);
}
                       ...
                       float projectionMatrix[16])
{
	...
	VSE_UniformTypeValuePair *typeValuePair = VSE_DictionaryGet(renderer->defaultMaterial->materialUniforms,
	                                                    "projection");
	typeValuePair->uniformValue = projectionMatrix;
...
{
	VSE_DrawGizmoRect(engine->renderer, position.x - size.x / 2, position.y - size.y / 2, size.x,
	              thickness);

	VSE_DrawGizmoRect(engine->renderer, position.x - size.x / 2, position.y + size.y / 2, size.x,
	              thickness);

	VSE_DrawGizmoRect(engine->renderer, position.x - size.x / 2, position.y - size.y / 2, thickness,
	              size.y);
...
static void RenderDebugInfo(VSE_Engine *engine, float projectionMatrix[16])
{
	...
	VSE_RenderDynamicText(engine->textAtlas, fpsText, (VSE_Vector2Float){10, 10}, (VSE_Vector2Float){1, 1},
...
static void RenderGizmo(VSE_Engine *engine, VSE_GizmoEntity *gizmoEntity, float projectionMatrix[16])
{
	...
	GLint colorLocation = glGetUniformLocation(engine->renderer->defaultGizmoMaterial->shaderProgram,
	                                           "color");
	...
	VSE_SpriteRenderer *image = VSE_GetComponent(connected, VSE_COMPONENT_SPRITE_RENDERER);
	...
	VSE_RenderHollowGizmoRect(engine, position, image->size, gizmoEntity->thickness);
	...
	VSE_RenderDynamicText(engine->textAtlas, entityInfo,
	                      ...
	                      }, (VSE_Vector2Float){1, 1}, VSE_HORIZONTAL_NOT_CENTERED,
	                      engine->renderer, projectionMatrix);
...
static void ApplyPostProcessing(VSE_Engine *engine, VSE_Window *window)
{
	if (engine->renderer->postProcessingEffects->totalEntries == 0)
	{
		...
		glUseProgram(engine->renderer->defaultPostProcessingMaterial->shaderProgram);
	...
	{
		for (int i = 0; i < engine->renderer->postProcessingEffects->allPairs->size; i++)
		{
			if (i == engine->renderer->postProcessingEffects->allPairs->size - 1)
			{
				glBindFramebuffer(GL_FRAMEBUFFER, 0);
				glClearColor(1.0f, 0.0f, 1.0f, 1.0f);
				glClear(GL_COLOR_BUFFER_BIT);
			...
			VSE_KeyValuePair *pair = VSE_DictionaryGetPair(engine->renderer->postProcessingEffects, i);
...
void VSE_UpdateMaterialUniforms(VSE_Material *material)
{
	for (int i = 0; i < material->materialUniforms->allPairs->size; i++)
	{
		...
		VSE_SetShaderUniform(material->shaderProgram, uniformName, typeValuePair->uniformType,
...
void VSE_UpdateRenderer(void *data, VSE_Engine *engine, float deltaTime)
{
	for (int i = 0; i < engine->allWindows->size; i++)
	{
		...
		VSE_CalculateProjectionMatrix(projectionMatrix, window->size.x, window->size.y);
		...
		for (int j = 0; j < window->gameEntitiesDrawList->size; j++)
		{
			...
			switch (window->renderType)
			{
				...
				case VSE_WINDOW_WORLD_SPACE:
					screenPos = VSE_WorldToScreen(worldPosition, window->viewportOffset);
					break;
			...
			RenderSprite(engine, screenPos, entity, sprite, projectionMatrix);
		}
		...
		for (int j = 0; j < window->uiEntitiesDrawList->size; j++)
		{
			...
			VSE_DynamicText *dynamicText = VSE_GetComponent(entity, VSE_COMPONENT_DYNAMIC_TEXT);

			if (dynamicText != NULL)
			{
				VSE_Vector2Float renderScale = {
					engine->pixelsPerUnit * entity->transform.scale.x * rect->parentScale.x,
					engine->pixelsPerUnit * entity->transform.scale.y * rect->parentScale.y
				};

				VSE_RenderDynamicText(engine->textAtlas, dynamicText->text,
				                      entity->transform.position, renderScale,
				                      VSE_HORIZONTAL_CENTERED, engine->renderer, projectionMatrix);
				continue;
			}
		...
		if (engine->debugMode)
		{
			...
			{
				VSE_GizmoEntity *gizmoEntity = VSE_ListGet(window->gizmosEntitiesDrawList, k);
